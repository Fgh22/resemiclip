...
void RadiusFlash_TraceLine(IReGameHook_RadiusFlash_TraceLine *chain, CBasePlayer *pPlayer, entvars_t *pevInflictor, entvars_t *pevAttacker, Vector& vecSrc, Vector& vecSpot, TraceResult *ptr)
{
	...
	for (int i = 0; i < g_GameData.GetMaxClients(); i++)
	{
		...
		if (!pHost->GetSolid(pTarget->GetIndex())) {
			// If the blocking player is at the same position as the victim (stacked on them),
			// they don't actually block the path — advance past them once before stopping.
			if (GET_DISTANCE(pHit->v.origin, vecSpot) < 32.0f) {
...
public:
	...
	void SetGroupMask()
	{
		for (int i = 1; i <= g_GameData.GetMaxClients(); ++i)
		{
			...
			// Check GetSolid from either direction, or fall back to distance check
			// (GetSolid may not be set yet if players just stacked without moving)
			bool bSolid = (pGamePlayer && pGamePlayer->GetSolid(m_iIndex)) || m_pAttacker->GetSolid(i);

			if (!bSolid && GET_DISTANCE(m_pev->origin, pPlayer->pev->origin) < g_Config.GetDistance()) {
				bSolid = true;
			}
